...
const RelatedProducts = ({subCategoryID}) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/Item?CategorySubID=${subCategoryID}`, {
                    headers: {
  ...
  return (
    <div className='w-full min-h-[350px] pt-10 md:mt-0 pb-10 bg-gradient-to-r from-purple-700 to-blue-500'>
        <h1 className='text-center font-bold text-3xl md:text-4xl font-poppins text-white uppercase'>Related products</h1>
        <div className='w-[90%] sm:w-[70%] mx-auto grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 mt-7'>
            {items.map((item) => (
                <div className='w-full h-[310px] border border-black cursor-pointer relative bg-white'>
                    <Link to={`/product/${item.itemID}`} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                        <img src={productImages[item.itemID]} className='w-full h-[280px] object-contain p-5' alt="" />
