...
const Product = () => {
        ...
        const fetchImageData = async () => {
            try {
                ...
                if (data.data.length > 0) {
                    setMainImage(`${process.env.REACT_APP_IMG_URL}/${data.data[0].imageID}.png`);
                }
            } catch (error) {
    ...
    return (
        <div className='pt-20 min-h-screen font-karla'>
            {/* Banner */}
            <div className='w-full h-[100px] sm:h-[150px] md:h-[200px] lg:h-[260px] bg-gradient-to-r from-purple-700 to-blue-500'>
            ...
            <div className='grid md:grid-cols-2 md:w-[90%] lg:w-[70%] xl:w-[60%] mx-auto gap-6 mt-10 font-karla pb-20'>
                <div>
                    <div className="relative">
                        ...
                        {zoomLens.show && (
                            <div 
                                ...
                                style={{
                                    left: `calc(${zoomLens.x}% - 75px)`,
                                    top: `calc(${zoomLens.y}% - 75px)`,
                    ...
                    <div className='flex items-center justify-center mt-4 gap-5 flex-wrap'>
                        {images.map((img, idx) => (
                            ...
                            alt={`Gallery ${idx}`} 
                            onClick={() => setMainImage(`${process.env.REACT_APP_IMG_URL}/${img.imageID}.png`)} // Update main image on click
                            />
                        ))}
                    </div>
                </div>
                ...
                <div className='w-[90%] mx-auto md:w-full h-full'>
                    ...
                    {/* Order Form */}
                    <p className='mt-4 font-bold text-2xl mb-2 text-blue-700'>Order {item?.itemName}</p>
                    <form className="flex flex-col gap-2">
                        <input type="text" placeholder='Name' className="border p-2 focus:border-amber-500 outline-none" />
                        <input type="text" placeholder='Contact Number' className="border p-2 focus:border-amber-500 outline-none" />
                        <input type="tel" placeholder='Address' className="border p-2 focus:border-amber-500 outline-none" />
                        <input type="number" placeholder='Quantity' className="border p-2 focus:border-amber-500 outline-none" />
                        <textarea placeholder='Type your request in detail' className="border p-2 focus:border-amber-500 outline-none" />
                        <button className="bg-gradient-to-r from-purple-700 to-blue-500 text-white py-2 px-4 rounded">Submit</button>
                    </form>
                </div>
            </div>
